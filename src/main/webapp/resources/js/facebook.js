	...
	window.fbAsyncInit = function() {
	    FB.init({
	      appId      : '1972378162977630', //내 앱 아이디
	      xfbml      : true,		
	      version    : 'v2.11'
	    });
			...
			if (response.status === 'connected') {
				...
				FB.api('/me','GET', {fields: 'email'}, function(member) { 
					...
					$.ajax({	
				    	  ...
				    	  data: {"email":fb_email}, // ,"fbaccesstoken":accessToken}, 
				    	  dataType: "json",
				    	  success : function(result) {		
									if(result == false){ //중복된 값을 타면 로그인이 된다. 
										...
										$.ajax({	
									    	  ...
									    	  url: "fbsignup.nn",
									    	  data: {"email":fb_email,
									    		     "fbaccesstoken":response.authResponse.accessToken}, // ,"fbaccesstoken":accessToken}, 
									    	  success: function(){
									    		  alert('회원가입이 되셨습니다 로그인 페이지 가서 해주세요 ');
